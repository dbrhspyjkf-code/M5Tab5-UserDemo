...
#if CONFIG_TAB5_XIAOZHI_OFFLINE_ONLY
    ESP_LOGW(TAG, "Tab5 xiaozhi offline mode: skip OTA and protocol startup");
#else
...
void Application::CheckAssetsVersion() {
    ...
    // If the server never pushed a wake word model (assets had none), fall back
    // to the locally-packed model in the "model" SPIFFS partition.
    if (!audio_service_.IsAfeWakeWord()) {
        audio_service_.SetLocalWakeWord();
    }
}
