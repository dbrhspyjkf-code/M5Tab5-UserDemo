...
Tab5AudioCodec::Tab5AudioCodec(void* i2c_master_handle, int input_sample_rate, int output_sample_rate,
    ...
    // ES8388 must be I2S SLAVE: the ESP32 I2S channel is created as
    // I2S_ROLE_MASTER (see CreateDuplexChannels), so it drives BCLK/WS. If the
    // ES8388 is also master_mode=true both fight over the clocks and the DAC
    // never gets a valid clock -> speaker is silent (mic still works because the
    // ES7210 defaults to slave). The BSP's bsp_audio_codec_speaker_init uses
    // master_mode=false for exactly this reason.
...
void Tab5AudioCodec::CreateDuplexChannels(gpio_num_t mclk, gpio_num_t bclk, gpio_num_t ws, gpio_num_t dout, gpio_num_t din) {
    ...
    // Smaller DMA footprint (was 6×240). The I2S slot reconfig done when the
    // wake-word path enables the mic input happens during peak memory pressure
    // (HA entity load + WiFi-over-SDIO storm at idle entry); a large DMA
    // descriptor allocation fails there and faults. 4×160 still gives ~26 ms of
    // buffering at 24 kHz, plenty for clean audio.
