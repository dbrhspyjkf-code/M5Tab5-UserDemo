...
void AudioService::SetLocalWakeWord() {
    // Create an AfeWakeWord without a pre-loaded model. When EnableWakeWordDetection()
    // is called later, AfeWakeWord::Initialize(codec_, nullptr) will call
    // esp_srmodel_init("model") itself to load from the "model" SPIFFS partition.
#if CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32P4
    if (wake_word_) {
        return;
    }
