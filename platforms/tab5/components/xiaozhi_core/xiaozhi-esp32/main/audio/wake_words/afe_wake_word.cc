...
bool AfeWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    // AEC disabled: the SR_HIGH_PERF AEC stage allocates large internal
    // DMA-capable buffers that exhaust internal RAM on the Tab5 (it shares the
    // pool with WiFi-over-SDIO + TLS-AES), crashing with sdio_rx_get_buffer /
    // i2s_alloc_dma_desc faults. Wake-word detection works fine without AEC;
    // echo cancellation only matters while the device's own TTS is playing.
    afe_config->aec_init = false;
    afe_config->aec_mode = AEC_MODE_SR_HIGH_PERF;
    afe_config->afe_perferred_core = 1;
    // The audio feed task runs at priority 8; if AFE's internal AEC/VAD/WakeNet
    // task stays at priority 1 it gets starved and the FEED ringbuffer overflows
    // ("Ringbuffer of AFE(FEED) is full"). Raise it so processing keeps up.
