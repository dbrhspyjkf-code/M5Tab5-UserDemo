...
// =============================================================================
// 5x7 LED 字体 — 已拆到 app_unit_puzzle/font5x7.{h,c} 共享 (自选股 LED ticker 也用).
// 格式见 font5x7.h. v6 起放 rodata, 避开 RMT 写 LED 期间 PSRAM 字形被覆盖的崩溃.
// =============================================================================
#include "font5x7.h"
...
{
    if (!_strip) return;
    // 不用 led_strip_clear() —— 它内部 memset+refresh 会先把灯全灭再刷, 与随后的
    // _show() 形成「黑一下→画图」两次刷新 = 动画/文字模式闪. 只清缓冲, 由 _show() 刷一次.
